...
def rle_decode(mask_rle: str, shape: tuple[int, int]) -> npt.NDArray:
    ...
    if mask_rle == '-' or mask_rle == 'authentic':
...
def _to_binary_mask(x: npt.NDArray, threshold: float = 0.5) -> npt.NDArray:
    """
    Her türlü maskeyi 0/1 binary'e çevirir:
    - bool -> 0/1
    - 0/255 -> 0/1
    - float prob -> threshold ile 0/1
    - (1,H,W) -> squeeze -> (H,W)
    """
    x = np.squeeze(x)
    ...
    # int/uint ise: >0 olanları 1 say
    return (x > 0).astype(np.uint8)


def calculate_f1_score(pred_mask: npt.NDArray, gt_mask: npt.NDArray, threshold: float = 0.5) -> float:
    pred = _to_binary_mask(pred_mask, threshold=threshold).flatten()
    gt = _to_binary_mask(gt_mask, threshold=threshold).flatten()
    ...
    if tp == 0 and fp == 0 and fn == 0:
        # ikisi de tamamen boşsa: mükemmel eşleşme
        return 1.0
